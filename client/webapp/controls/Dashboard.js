...
import GarageDoorTile from "../components/dashboard/GarageDoorTile";
    ...
    return (
      ...
      >
        {this.tiles.map(tile => {
          switch (tile.type) {
            case "garagedoor":
              return <GarageDoorTile key={++key} config={tile} />;
            case "clock":
              return <ClockTile key={++key} />;
            case "weather":
              return <WeatherTile key={++key} />;
            case "pool":
              return <PoolTile key={++key} />;
            case "spa":
              return <SpaTile key={++key} />;
            case "thermostat":
              return <ThermostatTile key={++key} device={tile.device} />;
            case "macro":
              return <MacroTile key={++key} config={tile} />;
            case "theater":
              return <TheaterTile key={++key} config={tile} />;
            case "smartthings":
              return (
                <div key={++key}>
            ...
            case "link":
              return (
                <LinkTile key={++key} href={tile.href} label={tile.label} />
              );
            default:
      ...
      <div style={{ marginTop: 2 }}>
        {this.tiles.map(tile => {
          switch (tile.type) {
            ...
            case "link":
              return (
                <LinkTile key={++key} onClick={tile.href} label={tile.label} />
              );
            default:
              console.log("invalid tile", tile);
              return null;
