...
export default class SmartThings extends React.Component {
  ...
  renderThing(thing, room, ndx) {
    const key = room + '-' + thing.name + '-' + ndx,
          level = this.state[`${thing.name}/level`],
      ...
      case 'dimmer':
        ...
        return (
          <DimmerField
            ...
            toggled={toggled}
            value={level}
            onToggle={::this.onDimmerToggled}
            onValueChange={::this.onDimmerChange}
          />
        )
      case 'fan':
        return (
  ...
  onDimmerChange(name, value, /* field */) {
    if (Number(name) === name) {
      this.control(`${name}/level`, name)
    }
